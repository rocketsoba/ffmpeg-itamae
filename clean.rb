...
if [ ! -d /home/#{node["userdata"]["name"]}/opt/x264 ]; then
  env PATH=$PATH:/home/#{node["userdata"]["name"]}/opt/yasm/bin ./configure --prefix=/home/#{node["userdata"]["name"]}/opt/x264 --enable-static --enable-shared
...
if [ ! -d /home/#{node["userdata"]["name"]}/opt/lame ]; then
  env LDFLAGS="-static" ./configure --prefix=/home/#{node["userdata"]["name"]}/opt/lame --enable-shared --enable-static
  make -j4;
  make install;
fi
EOH
...
if [ ! -d /home/#{node["userdata"]["name"]}/opt/fdk-aac ]; then
  autoreconf -i
  ./configure --prefix=/home/#{node["userdata"]["name"]}/opt/fdk-aac --enable-shared --enable-static
  make -j4;
  make install;
fi
EOH
  not_if "env PATH=$PATH:/home/#{node["userdata"]["name"]}/opt/fdk-aac/bin which fdk-aac"
end
